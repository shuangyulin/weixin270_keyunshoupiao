<template>
  <div class="main-content">
		     ...
		     <template slot-scope="scope">
              ...
              <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('dingdanxinxi','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                                                                <el-button v-if="isAuth('dingdanxinxi','退票登记') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="tuipiaoxinxiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'退票登记':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','退票登记') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="tuipiaoxinxiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'退票登记':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','退票登记') && contents.tableBtnIcon == 0" type="success" size="mini" @click="tuipiaoxinxiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'退票登记':'' }}</el-button>
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
            ...
            <tuipiaoxinxi-cross-add-or-update v-if="tuipiaoxinxiCrossAddOrUpdateFlag" :parent="this" ref="tuipiaoxinxiCrossaddOrUpdate"></tuipiaoxinxi-cross-add-or-update>
      ...
      width="800">
                                    <div id="banciChart" style="width:100%;height:600px;"></div>
                                                                                                                                                                                                                                                                                <span slot="footer" class="dialog-footer">
...
import tuipiaoxinxiCrossAddOrUpdate from "../tuipiaoxinxi/add-or-update";
export default {
  data() {
    return {
                                                                                                                                                                                                                                                                                                                                                                                                searchForm: {
      ...
      addOrUpdateFlag:false,
                  tuipiaoxinxiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    contentPageStyleChange(){
      ...
      this.contents.pageEachNum = 10
            ...
            tuipiaoxinxiCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.tuipiaoxinxiCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','dingdanxinxi');
      this.$nextTick(() => {
        this.$refs.tuipiaoxinxiCrossaddOrUpdate.init(row.id,type);
      });
    },
                payHandler(row){
      ...
      this.$nextTick(()=>{
                                                                  var banciChart = this.$echarts.init(document.getElementById("banciChart"),'macarons');
        this.$http({
            url: "group/dingdanxinxi/banci",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
            let res = data.data;
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
                    ...
                    kechechepaihao: row.kechechepaihao,
                    chufariqi: row.chufariqi,
                    chufashijian: row.chufashijian,
                    qidian: row.qidian,
                    zhongdian: row.zhongdian,
                    zuoweileixing: row.zuoweileixing,
                    piaojia: row.piaojia,
                    piaoshu: row.piaoshu,
                    zongjiage: row.zongjiage,
                    goupiaoneirong: row.goupiaoneirong,
