	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(160, 156, 156, 0.82)"}'>
			<view @tap="onDetailTap(product)" class="uni-product" :style='{"borderRadius":"8rpx","backgroundColor":"#fff"}' v-for="(product,index) in list" :key="index">
					...
					<text v-if="isAuth('shiwuzhaoling','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('shiwuzhaoling','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
				</view>
			</view>
		</view>
		
		
		<button v-if="isAuth('shiwuzhaoling','新增')" class="add-btn" @click="onAddTap()">新增</button>
			...
			async search(){
				...
				let searchForm = {
					...
					searchForm['leixing'] = '%' + this.searchForm.leixing + '%'
																																																				...
																																																				let res = await this.$api.list(`shiwuzhaoling`, searchForm);
				// 如果是第一页数据置空
